...
\htmladdnormallink{Polish}{http://jarek.katowice.pl/jcwww/pbntools/help_pl.html}.
...
\item[Download Tournament Calculator]
  Downloads hands records from Polish Tournament Calculator system.
\item[Deal cards]
...
under \htmladdnormallink{GNU~GPL}{http://jarek.katowice.pl/jcwww/pbntools/gpl30.html} license,
version 3 or later.
...
\item[wget]
  ...
  belonging to \htmladdnormallink{Msys2}{https://www.msys2.org/},
  is a tool for recursive download of web pages, together with all needed files.
  License files: wget\_COPYING.txt, wget\_README.txt.
...
\subsubsection{Tournament download}

Use option {\em Download hands} / {\em Hands type: Bbo}.
...
\subsubsection{Hands history download}

Use option {\em Download hands} / {\em Hands type: Bbo Hand records}.

Hands history link should be used, obtained from {\em Hand records} screen by {\em Get hands} button.
Example link:
\begin{verbatim}
...
%{{{ download Tournament Calculator
\subsection{Download Tournament Calculator} \label{pobTourCalc}

  {\em Tournament Calculator} is current software used in Polish tournaments,
  like \htmladdnormallink{MZBS}{https://mzbs.pl/wyniki}.
  It replaced Kops and Pary.
  For download use the main link for results,
  e.g. \htmladdnormallink{260330}{https://mzbs.pl/files/2021/wyniki/zs/260330/}.
%}}}
...
\item[Internet video camera]
  ...
  Autofocus cameras also work, including those built in laptops.
  
\item[Barcoded cards]
  Two formats of barcodes are recognized:
  
  \begin{description}
  
  \item[PCard]
  Also known as WIN. You can recognize it by black narrow stripes
  surrounding the code. The number of stripes varies, but it's always more than 3.
  
  \item[PCard2]
  Currently, that is in 2026, it's the only standard possible to obtain.
  Every code consists of 4 stripes of width 1 or 2.
  You can buy it in \bridgepollink.
  
  \end{description}
  
\item[Zbarcam parameters]

  Zbarcam parameters consist of built-in program defaults and user-configured
  options. They are visible in the dealing process in the textual window.

  PCard and PCard2 modes are mutually exclusive; reading both at once may
  lead to false detections. By default only one standard is active; you can
  override that using your configuration.

  Default parameters are not always optimal. Scanning may be slow or produce
  false reads. You can speed up scanning using \verb!prescale! and reduce false
  positives using \verb!uncertainty!.

  \begin{description}

    \item[enable/disable]

    Enable or disable a barcode standard. Last occurrence wins.
    Example: \verb!-Spcard2.disable! will disable PCard2 standard.

    \item[prescale]

    Camera often produces high resolution images which are unnecessary and
    slow scanning. You may rescale the image, e.g. \verb!--prescale=640x480!.
    Provided dimensions must be supported by the camera as one of its modes.

    \item[uncertainty]

    Number of consecutive camera frames on which the barcode must appear.
    Higher value increases certainty but takes proportionally longer.
    Minimum value is 1. Default is 2.
    Example: \verb!-Spcard2.uncertainty=3!
...
\item[Zbarcam options]
  ...
  To select which cards to use, you should disable the ones you don't have.
  Initially both PCard and PCard2 is enabled. 
  You can add for example \verb!-Spcard.disable!, then only PCard2 will be active.
...
\item
  ...
  \begin{itemize}
  \item
    \htmladdnormallink{zbarw modified}{http://jarek.katowice.pl/jcwww/pbntools/%
...
\item
  wget sources available as other sources distributed with msys
...
\item[1.6.0, 12.09.2026]
  \begin{itemize}
  \item Converting BBO (lin) tiny urls to PBN.
  \item New zbarcam with scanning improvements (quality threshold, uncertainty).
  \item New version of Tournament Calculator, which supports gz files.
  \end{itemize}
\item[1.5.0, 25.06.2026]
  \begin{itemize}
  \item Dealing cards with barcodes PCard2.
  \end{itemize}
\item[1.4.0, 01.05.2026]
  \begin{itemize}
  \item Tournament Calculator format.
  \end{itemize}
\item[1.3.3, 22.05.2021]
  \begin{itemize}
  \item Download improvement - automatic retry after http error 503
  \end{itemize}
\item[1.3.2, 20.05.2021]
  \begin{itemize}
  \item Fixed problem with downloading Bbo hand records
  \item Upgraded wget for windows to 1.20.3-2 by msys
  \end{itemize}
