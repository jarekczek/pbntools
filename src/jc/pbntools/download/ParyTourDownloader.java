...
public class ParyTourDownloader extends HtmlTourDownloader
{
  ...
  private void readScoring(Deal deal, Document doc)
  {
    ...
    if (elems.size() >= 2) {
      // potrzebny nam jest drugi wiersz tabelki, nagłówkowy
      Elements tds = elems.get(1).select("td");
      if (tds.size() == 8) {
        // musi mieć 8 kolumn, pierwsza jest niewidoczna
    ...
    throws DownloadFailedException
  {
    ...
    for (Element tr: elems) {
      ...
      // w 2. kolumnie powinien być numer pary, więc tylko te wiersze
      // będziemy czytać
